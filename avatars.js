(() => {
 ...
 function make(name,avatar){
  const chosen=/^avatar-(?:[1-9]|1[0-2])$/.test(avatar||'')?Number(avatar.split('-')[1]):0;
  const key=String(name||'').trim().toLowerCase(),boy=chosen?chosen<=6:boys.has(key);
  let seed=0;for(const ch of key)seed=(seed*31+ch.codePointAt(0))>>>0;
  if(chosen)seed=chosen-1;
 ...
 window.LoopShiftAvatar={make,choices:Array.from({length:12},(_,i)=>'avatar-'+(i+1))};
})();
