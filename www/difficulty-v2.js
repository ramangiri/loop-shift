...
(() => {
 ...
 const originalAddRow=addRow,originalUpdateRow=updateRow,originalDrawGuide=drawGuide,originalWarnPattern=warnPattern,previousShift=shift;
 ...
 const safeStep=(from,step)=>{
   let next=clampLane(from+step);
   if(step&&next===from)next=clampLane(from-step);
   return next;
 };
 ...
 function makeAttack(row,prevSafe){
   const local=localIndex(row),block=Math.floor(local/3),seq=block%2===0?[1,1,-1]:[-1,1,0],labels=block%2===0?'OUT → OUT → IN':'IN → OUT → HOLD';
 ...
 function configureBoss(row,prevSafe){
   ...
   else {row.pattern=local%2?'pulse':'moving';row.locked=false;if([9,11].includes(local)){const options=twoAway(prevSafe);if(options.length)laneSet(row,options[(row.index+level)%options.length]);}if(local===10){row.switching=true;row.switchFromLane=row.sparkLane;const c=[row.sparkLane-1,row.sparkLane+1].filter(n=>n>=0&&n<ringCount);row.switchToLane=c[(row.index+level)%Math.max(1,c.length)]??row.sparkLane;}}
 ...
 addRow=function(a,index=nextRowIndex++){
   ...
   if(level>=21&&level<=30&&[3,7,10].includes(local)){
     row.switching=true;row.switchFromLane=row.sparkLane;const c=[row.sparkLane-1,row.sparkLane+1].filter(n=>n>=0&&n<ringCount);row.switchToLane=c[(row.index+level)%Math.max(1,c.length)]??row.sparkLane;row.pattern='moving';row.locked=false;row.patternStart=true;
   }
   ...
   if(level>=41&&level<=60){
     ...
     if([4,9].includes(local)){row.switching=true;row.switchFromLane=row.sparkLane;const c=[row.sparkLane-1,row.sparkLane+1].filter(n=>n>=0&&n<ringCount);row.switchToLane=c[(row.index+level)%Math.max(1,c.length)]??row.sparkLane;row.pattern='moving';row.locked=false;}
   }
   if(level>=61){
     ...
     if(perfectStreak>=5&&[4,9].includes(local)){row.switching=true;row.switchFromLane=row.sparkLane;const c=[row.sparkLane-1,row.sparkLane+1].filter(n=>n>=0&&n<ringCount);row.switchToLane=c[(row.index+perfectStreak)%Math.max(1,c.length)]??row.sparkLane;row.pattern='moving';row.locked=false;}
   }
   ...
   if(before){
     ...
     else if(level>=41&&level<=60)scale=.92;
     else if(level>=61)scale=perfectStreak>=6?.80:perfectStreak>=3?.86:.94;
     if(row.bossPhase===3)scale=Math.min(scale,.85);
 ...
 updateRow=function(row){
   ...
   if(row?.switching&&!row.switchDone){const seconds=(row.baseAngle-angle)/Math.max(.01,speedNow());if(seconds<=1.22){row.switchDone=true;laneSet(row,row.switchToLane,Number.isInteger(row.riskLane)?row.riskLane:null);row.sparkLane=row.switchToLane;row.locked=true;row.angle=row.baseAngle;showEffect('GAP SWITCH · MOVE!','close');tone(360,.08,'triangle',.04,640);}}
   return result;
 };
 ...
 function makeRecovery(){
   ...
   for(const row of future){row.recovery=true;row.switching=false;row.switchDone=true;row.doubleShift=false;row.riskLane=null;row.riskReward=false;row.pattern='classic';row.locked=true;row.open=false;safe=clampLane(row.entryLane??safe);laneSet(row,safe);}
 }
 function frame(){
   ...
   const guide=$('guide-status');if(guide&&level>10&&!focusRun&&roundKind!=='tutorial'&&roundKind!=='practice'){
     const target=guidedTarget();guide.textContent=guideVisible()?(target===lane?'HOLD · Stay on this ring':`BLUE FLASH · Ring ${target+1}`):'READ THE PATTERN · Guide hidden';
   }
