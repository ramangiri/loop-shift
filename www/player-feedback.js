...
/* Gameplay engagement layer: optional special collectibles, 10-level identities and lightweight checkpoint difficulty feedback. */
(() => {
 ...
 const zones=['FOUNDATION','RHYTHM','CHOICE','FLOW','PRECISION','PRESSURE','MASTERY','FOCUS','VELOCITY','FINAL LOOP'];let lastLevel=0,lastBreak=false,serial=0;const seen=new WeakSet();const stats={gem:0,star:0,crown:0};try{Object.assign(stats,JSON.parse(localStorage.getItem('loop-shift-specials-v1')||'{}'));}catch{}
 const style=document.createElement('style');style.textContent=`.special-coin-layer{position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:5}.special-coin-legend{margin:.7rem 0;padding:.75rem;border:1px solid color-mix(in srgb,currentColor 18%,transparent);border-radius:14px;font-size:.86rem;line-height:1.45}.special-coin-legend strong{display:block;margin-bottom:.3rem}.special-coin-legend span{white-space:nowrap}.difficulty-pulse{position:fixed;left:50%;bottom:max(18px,env(safe-area-inset-bottom));transform:translateX(-50%);z-index:80;background:#15201c;color:#f5f7f6;border:1px solid #53655d;border-radius:18px;padding:12px 14px;width:min(92vw,430px);box-shadow:0 14px 42px #0008}.difficulty-pulse p{margin:0 0 9px;font-weight:700}.difficulty-pulse div{display:flex;gap:8px}.difficulty-pulse button{flex:1;min-height:42px;border-radius:12px;border:1px solid #71877c;background:#1d2a24;color:inherit;font:inherit}.difficulty-pulse button:active{transform:scale(.98)}`;document.head.appendChild(style);
 const gameCanvas=$id('game');const layer=document.createElement('canvas');layer.className='special-coin-layer';layer.setAttribute('aria-hidden','true');if(gameCanvas?.parentElement){const parent=gameCanvas.parentElement;if(getComputedStyle(parent).position==='static')parent.style.position='relative';parent.appendChild(layer);}const lctx=layer.getContext('2d');
 function specialForLevel(lvl){const roll=(serial*37+lvl*17)%100;if(lvl>=21&&roll<16)return'crown';if(lvl>=11&&roll<48)return'star';return'gem';}function chooseLane(row){return Number.isInteger(row.bonusLane)?row.bonusLane:row.sparkLane;}
 ...
 function addGuide(){const list=document.querySelector('.essential-rules');if(!list||document.getElementById('special-coin-guide'))return;const li=document.createElement('li');li.id='special-coin-guide';li.innerHTML='<strong>Choose special collectibles.</strong> Purple gem <b>+100</b>, cyan star <b>+250</b>, golden crown <b>+500</b>. They are optional. The blue arc is visual guidance only.';list.appendChild(li);const legend=document.createElement('div');legend.className='special-coin-legend';legend.innerHTML='<strong>SPECIAL COLLECTIBLES</strong><span>◆ Purple Gem +100</span> · <span>★ Cyan Star +250</span> · <span>♛ Golden Crown +500</span><br><small>Gem from Level 1 · Star from Level 11 · Crown from Level 21</small>';list.parentElement.appendChild(legend);}
...
/* Controls: blue is guidance only. A normal tap cycles one adjacent ring in the
   current travel direction; at an edge it reverses. Blue never forces movement
   and there is no HOLD state. */
(() => {
 const originalShift=shift;
 shift=function(direction=0){
  if(trainingWaiting||screen!=='game'||mode!=='playing'||startDelay>0||gameTime-lastShift<.055)return;
  if(direction){if(gameTime-lastShift<.095)lastShift=gameTime-.096;return originalShift(direction);}
  let step=shiftDirection<0?-1:1;
  if(lane<=0)step=1;else if(lane>=ringCount-1)step=-1;
  if(gameTime-lastShift<.095)lastShift=gameTime-.096;
  return originalShift(step);
 };
 const guide=$id('guide-status');
 function visualOnly(){
  if(!guide)return;
  const target=guidedTarget();
  guide.textContent=target===lane?'BLUE GUIDE · Safe ring':'BLUE GUIDE · Recommended ring '+(target+1);
  $('shift')?.setAttribute('aria-label','Tap to move one ring. Blue arc is guidance only.');
  requestAnimationFrame(visualOnly);
 }
 requestAnimationFrame(visualOnly);
})();