...
export async function dailyBoard(db,id,day=dailyInfo().day){
  ...
  const profile=id?await db.prepare('SELECT name,selected_title,highest_level,furthest_pass,achievements,best_chain,best_clean FROM players WHERE id=?').bind(id).first():null;
  ...
  return {challenge:dailyInfo(Date.parse(day+'T12:00:00Z')),entries:results.map((p,i)=>({rank:i+1,name:p.name,title:p.selected_title,score:p.best,isYou:p.player_id===id})),me:profile?{key:id,name:profile.name,title:profile.selected_title,best:mine?.best||0,rank,progress:{highest:profile.highest_level,distance:profile.furthest_pass,badges:profile.achievements,chain:profile.best_chain,clean:profile.best_clean}}:null};
}
