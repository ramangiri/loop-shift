...
export async function weeklyBoard(db,id,week=weeklyInfo().week){
  ...
  const profile=id?await db.prepare('SELECT name,selected_title FROM players WHERE id=?').bind(id).first():null;
  ...
  return {challenge:weeklyInfo(Date.parse(week+'T12:00:00Z')),entries:results.map((p,i)=>({rank:i+1,name:p.name,title:p.selected_title,score:p.best,isYou:p.player_id===id})),me:profile?{key:id,name:profile.name,title:profile.selected_title,best:mine?.best||0,rank}:null};
}
